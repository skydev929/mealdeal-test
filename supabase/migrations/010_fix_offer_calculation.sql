-- Fix offer price calculation
-- The issue: offers might not be matching correctly or calculation is wrong
...
-- Diagnostic query to check offers
-- Run this to see if offers are being found:
/*
SELECT 
  di.dish_id,
  di.ingredient_id,
  ...
  o.valid_to,
  CASE 
    WHEN o.offer_id IS NOT NULL THEN 'HAS OFFER'
    ELSE 'NO OFFER'
  END as offer_status
FROM dish_ingredients di
JOIN ingredients i ON di.ingredient_id = i.ingredient_id
