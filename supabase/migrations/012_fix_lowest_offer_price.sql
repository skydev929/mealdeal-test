-- Fix calculate_dish_price to select lowest price offer when multiple offers exist for same ingredient
-- This ensures dish cards show the best available price
...
DECLARE
  _region_id TEXT;  -- Changed from INTEGER to TEXT (region_id is now TEXT after migration 016)
...
BEGIN
  ...
  IF _region_id IS NOT NULL THEN
    -- Calculate offer price for each ingredient, selecting the LOWEST price offer
      ...
      SELECT 
        ...
        CASE 
          ...
          ELSE
            0
        END,
        -- Offer price (if available) - select LOWEST price offer
        COALESCE((
          SELECT MIN(
            CASE
          ...
          FROM offers o
          WHERE o.ingredient_id = di.ingredient_id
