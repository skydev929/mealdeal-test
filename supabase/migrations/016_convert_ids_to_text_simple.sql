-- Migration: Convert chain_id, region_id, and store_id from INTEGER/SERIAL to TEXT
-- This allows for more flexible ID formats similar to dish_id (e.g., C001, R001, S001)
-- Since database is empty, we can directly alter column types
...
-- ============================================================================
-- Step 2: Drop primary key constraints and sequences
-- ============================================================================

-- Drop primary key from chains (chain_id is SERIAL, so we need to drop the sequence too)
ALTER TABLE chains DROP CONSTRAINT IF EXISTS chains_pkey;
DROP SEQUENCE IF EXISTS chains_chain_id_seq CASCADE;

-- Drop primary key from ad_regions
ALTER TABLE ad_regions DROP CONSTRAINT IF EXISTS ad_regions_pkey;

-- Drop primary key from stores
ALTER TABLE stores DROP CONSTRAINT IF EXISTS stores_pkey;

-- Drop primary key from store_region_map (composite key)
ALTER TABLE store_region_map DROP CONSTRAINT IF EXISTS store_region_map_pkey;

-- ============================================================================
-- Step 3: Alter column types from INTEGER/SERIAL to TEXT
-- ============================================================================
-- Note: USING clause is required for type conversion in PostgreSQL
...
-- Recreate primary key for chains
ALTER TABLE chains ADD PRIMARY KEY (chain_id);

-- Recreate primary key for ad_regions
ALTER TABLE ad_regions ADD PRIMARY KEY (region_id);

-- Recreate primary key for stores
ALTER TABLE stores ADD PRIMARY KEY (store_id);

-- Recreate composite primary key for store_region_map
ALTER TABLE store_region_map ADD PRIMARY KEY (store_id, region_id);

-- ============================================================================
-- Step 5: Recreate all foreign key constraints with CASCADE options
-- ============================================================================
...
-- ============================================================================
-- Step 6: Update the calculate_dish_price function to use TEXT for region_id
-- ============================================================================
